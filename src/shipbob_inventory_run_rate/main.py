...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Calculate current recent run rate (exponentially weighted mean) & current inventory on hand for all products sold in the past 90 days')
    ...
    shipbob_order_details_df = run_athena_query(query, database, region, s3_bucket)
    ...
    shipbob_inventory_details_df = run_athena_query(query, database, region, s3_bucket)
    
    
    # Update data types
    shipbob_inventory_details_df['partition_date'] = pd.to_datetime(
        shipbob_inventory_details_df['partition_date']).dt.strftime('%Y-%m-%d')
    shipbob_inventory_details_df['inventory_id'] = shipbob_inventory_details_df['id'] 
    shipbob_inventory_details_df['total_fulfillable_quantity'] = shipbob_inventory_details_df['total_fulfillable_quantity'].fillna(0).astype(int) 
    ...
    for product in daily_qty_sold_df['inventory_id'].unique():
        ...
        current_run_rate_df = pd.concat([current_run_rate_df, pd.DataFrame(
            {'inventory_id': [product], 'run_rate': [ewm_series.iloc[-1]]})])
    
    
    # Join run rate data with inventory data
    inventory_cols = ['inventory_id','name','total_fulfillable_quantity']    # column subset to merge with run rate data
    daily_metrics_df = current_run_rate_df.merge(shipbob_inventory_details_df[inventory_cols],how='left',on='inventory_id')
    
    # Calculate estimated days of stock onhand & estimated stockout date 
    daily_metrics_df['est_stock_days_on_hand'] = daily_metrics_df['total_fulfillable_quantity'] / daily_metrics_df['run_rate']
    daily_metrics_df['estimated_stockout_date'] = daily_metrics_df['est_stock_days_on_hand'].apply(lambda x: pd.to_datetime('today') + timedelta(x))
