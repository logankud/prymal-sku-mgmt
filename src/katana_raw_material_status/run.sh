...
MISSING_DATES=$(python3 src/gap_detector.py --table katana_raw_material_status --date_col partition_date --lookback_days 7 2>/dev/null)
if [ -n "$MISSING_DATES" ]; then
    echo "-------- Auto-backfill: missing dates detected: $MISSING_DATES"
    for dt in $MISSING_DATES; do
        echo "-------- Backfilling $dt"
        python src/katana_raw_material_status/main.py --partition_date $dt
    done
else
