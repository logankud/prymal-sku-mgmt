...
MISSING_DATES=$(python3 src/gap_detector.py --table shopify_active_variant_sku_details --date_col partition_date --lookback_days 7 2>/dev/null)
if [ -n "$MISSING_DATES" ]; then
    echo "-------- Auto-backfill: missing dates detected: $MISSING_DATES"
    for dt in $MISSING_DATES; do
        echo "-------- Backfilling $dt"
        python src/shopify_active_variant_sku_details/main.py --partition_date $dt
    done
else
